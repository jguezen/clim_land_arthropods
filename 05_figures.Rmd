...
fig2a <- ggplot(data = world) +
  ...
  coord_sf(
    xlim = c(
      min(climate_land_abund$Longitude) - 2,
      max(climate_land_abund$Longitude) + 2
    ),
    ylim = c(
      min(climate_land_abund$Latitude) - 2,
      max(climate_land_abund$Latitude) + 2
    ),
    expand = FALSE
  ) +
...
fig3_plot <- ggplot(
  data = Trait_bar,
  aes(y = percent,
      x = factor(dispersal, levels = c("wind", "flight/wind", "flight", "ground")),
      fill = diet)) +
  ...
  geom_hline(yintercept = 20, 
             linetype = "dashed") +
  facet_grid(
    factor(fg, levels = c("pollinators", "pests", "natural enemies")) ~ habitat, scale = "free_y") +
  labs(y = "% sites present") +
  scale_fill_manual(
    values = c("#f0f921", "#ed7953"), 
    labels = c("diet\ngeneralist", "diet\nspecialist"),
    drop = T) +
  theme_classic() +
  theme(
...
ci_graphic <- ci_graphic %>%
  ...
  mutate(
    term = case_when(
      ...
      term == "(change3_MAT)" ~ "MAT",
      term == "(change3_VAT)" ~ "VAT",
      term == "(semifor_l):(change3_MAT)" ~ " nat.hab*MAT",
      term == "(change3_MAT):(semifor_l)" ~ " nat.hab*MAT",
      term == "(semifor_l):(change3_VAT)" ~ " nat.hab*VAT",
      term == "(change3_VAT):(semifor_l)" ~ " nat.hab*VAT",
      term == "(change3_MAT):(change3_VAT)" ~ " MAT*VAT",
      term == "(change3_VAT):(change3_MAT)" ~ " MAT*VAT"
      ))
...
heatmap_01.1 <- ggplot() +
  ...
  theme(
    ...
    axis.title.y = element_text(size = 8, colour = "grey50"),
...
```{r}
plot_05_500_dat_hull <- climate_land_abund_05_500 %>%
  slice(chull(semifor_l, change3_MAT))

plot_05_500_dat_hull <- plot_05_500_dat_hull %>% dplyr::select(semifor_l, change3_MAT)

plot_05_500_dat_hull_sf <- plot_05_500_dat_hull %>%
  st_as_sf(coords = 
             c("semifor_l",
               "change3_MAT")) %>%
  summarise() %>%
...
plot_05_500_dat_xy <-
  with(climate_land_abund_05_500,
       expand.grid(
         semifor_l = seq(min(semifor_l),
                        max(semifor_l),
                        len = 300),
...
plot_05_500_dat_xy_sf = st_as_sf(
  plot_05_500_dat_xy,
  coords = c("semifor_l", "change3_MAT"))

plot_05_crop <-
  st_intersection(
    plot_05_500_dat_hull_sf,
    plot_05_500_dat_xy_sf) %>%
  st_coordinates() %>%
  as.data.frame() %>%
  rename(semifor_l = X, 
         change3_MAT = Y) %>%
  mutate(change3_VAT = mean(climate_land_abund_05_500$change3_VAT),
         StudyID_yr = NA,
         Sampling_method = NA,
         sampling_effort_scaled = 1)
...
plot_05_500_dat_hull <- plot_05_500_dat_hull %>% dplyr::select(semifor_l, change3_VAT)
...
plot_05_crop <-
  ...
  rename(semifor_l = X, 
         change3_VAT = Y) %>%
  mutate(change3_MAT = mean(climate_land_abund_05_500$change3_MAT),
         StudyID_yr = NA,
         Sampling_method = NA,
         sampling_effort_scaled = 1)
...
heatmap_08.1 <- ggplot() +
  ...
  theme(
    ...
    # plot.title = element_text(size = 8),
    axis.title.y = element_text(size = 8, face = "bold"),
    axis.title.x = element_text(size = 8, colour = "grey50"),
    axis.line = element_blank(),
    axis.title = element_text(size = 8),
    axis.text = element_text(size = 7),
    legend.text = element_text(size = 7),
...
```{r}
plot_10_2000_dat_hull <- climate_land_abund_10_2000 %>%
  slice(chull(semifor_l, change3_VAT))

plot_10_2000_dat_hull <- plot_10_2000_dat_hull %>% dplyr::select(semifor_l, change3_VAT)

plot_10_2000_dat_hull_sf <- plot_10_2000_dat_hull %>%
  st_as_sf(coords = 
             c("semifor_l",
               "change3_VAT")) %>%
  summarise() %>%
...
plot_10_2000_dat_xy <-
  with(climate_land_abund_10_2000,
       expand.grid(
         semifor_l = seq(min(semifor_l),
                        max(semifor_l),
                        len = 300),
...
plot_10_2000_dat_xy_sf = st_as_sf(
  plot_10_2000_dat_xy,
  coords = c("semifor_l", "change3_VAT"))

plot_10_crop <-
  st_intersection(
    plot_10_2000_dat_hull_sf,
    plot_10_2000_dat_xy_sf) %>%
  st_coordinates() %>%
  as.data.frame() %>%
  rename(semifor_l = X, 
         change3_VAT = Y) %>%
  mutate(change3_MAT = mean(climate_land_abund_10_2000$change3_MAT),
         StudyID_yr = NA,
         Sampling_method = NA,
         sampling_effort_scaled = 1)
...
```{r}
plot_11_250_dat_hull <- climate_land_abund_11_250 %>%
  slice(chull(semifor_l, change3_VAT))

plot_11_250_dat_hull <- plot_11_250_dat_hull %>% dplyr::select(semifor_l, change3_VAT)

plot_11_250_dat_hull_sf <- plot_11_250_dat_hull %>%
  st_as_sf(coords = 
             c("semifor_l",
               "change3_VAT")) %>%
  summarise() %>%
...
plot_11_250_dat_xy <-
  with(climate_land_abund_11_250,
       expand.grid(
         semifor_l = seq(min(semifor_l),
                        max(semifor_l),
                        len = 300),
...
plot_11_250_dat_xy_sf = st_as_sf(
  plot_11_250_dat_xy,
  coords = c("semifor_l", "change3_VAT"))

plot_11_crop <-
  st_intersection(
    plot_11_250_dat_hull_sf,
    plot_11_250_dat_xy_sf) %>%
  st_coordinates() %>%
  as.data.frame() %>%
  rename(semifor_l = X, 
         change3_VAT = Y) %>%
  mutate(change3_MAT = mean(climate_land_abund_11_250$change3_MAT),
         StudyID_yr = NA,
         Sampling_method = NA,
         sampling_effort_scaled = 1)
